...
class RagUtil:
    ...
    def index_data(
            self, document_directory: str = None, index_directory: str = None, chunk_size: int = 600,
        ...
        text_data_ds = ds.flat_map(self.do_some_data_extraction)

        # Chunk data
        chunks_ds = text_data_ds.flat_map(
            partial(
                self.do_some_data_chunking, chunk_size=chunk_size, chunk_overlap=chunk_overlap
            )
        )
    ...
    def do_chat_completion(
        ...
        query_agent = QueryAgent(
            ...
            faiss_index=faiss_index, metadata_dict=metadata_dict, lexical_index=lexical_index,
            chunks=chunks, reranker=reranker
        )
